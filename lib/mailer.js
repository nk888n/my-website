...
function addContactFooter(html){
  const base=siteUrl(),text=String(html),parts=[];
  if(!text.includes(business.name))parts.push(`<b>${esc(business.name)}</b>`);
  if(!text.includes(business.address))parts.push(esc(business.address));
  if(!text.includes(business.phone))parts.push(esc(business.phone));
  if(!text.includes(business.email))parts.push(`<a href="mailto:${esc(business.email)}">${esc(business.email)}</a>`);
  const hasExactWebsiteLink=text.includes(`href="${base}"`)||text.includes(`href='${base}'`);
  if(!hasExactWebsiteLink)parts.push(`<a href="${esc(base)}">Visit our website</a>`);
...
function dateTimeLabel(value){
  ...
  return new Intl.DateTimeFormat("en-CA",{timeZone:business.timezone,year:"numeric",month:"long",day:"numeric",hour:"numeric",minute:"2-digit",hour12:true}).format(d);
}
function dateLabel(value){
  if(!value)return "";
  const d=new Date(value);
  if(Number.isNaN(d.getTime()))return "";
  return new Intl.DateTimeFormat("en-CA",{timeZone:business.timezone,year:"numeric",month:"long",day:"numeric"}).format(d);
}
function timeLabel(value){
  if(!value)return "";
  const d=new Date(value);
  if(Number.isNaN(d.getTime()))return "";
  return new Intl.DateTimeFormat("en-CA",{timeZone:business.timezone,hour:"numeric",minute:"2-digit",hour12:true}).format(d);
}
...
export async function sendMail({to,subject,html}){
  ...
  try{
    return await transporter.sendMail({from:`"VALE BEAUTY VK" <${process.env.GMAIL_USER}>`,to,subject,html:customerHtml});
  }catch(error){
    if(subject.startsWith("Account Fee Notice")){console.error("FEE EMAIL FAILED:",error);return {accepted:[],rejected:[to],error};}
    throw error;
  }
}
