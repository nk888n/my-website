...
export default function ServicesClient({ sections, initialSelection }) {
  ...
  return <>
    <div className="grid">{sections.map(([title, items]) => <div key={title} style={{ gridColumn: "1/-1" }}>
      <h2>{title}</h2><div className="grid">{items.map(s => <Card key={s.id} s={s} selected={[sel.facial?.id, sel.body?.id, sel.eyebrow?.id].includes(s.id)} onSelect={pick} onRemove={remove} />)}</div>
    </div>)}</div>
