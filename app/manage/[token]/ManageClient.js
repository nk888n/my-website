...
export default function ManageClient({token}){
 const [email,setEmail]=useState("");const [booking,setBooking]=useState(null);const [msg,setMsg]=useState("");const [date,setDate]=useState("");const [start,setStart]=useState("");const [blocked,setBlocked]=useState([]);const [available,setAvailable]=useState([]);const [loading,setLoading]=useState(false);const [loadingSlots,setLoadingSlots]=useState(false);
 ...
 {booking&&<div className="manageDetails" style={{marginTop:20}}>
   <div className="summary"><strong>{booking.date} · {booking.startLabel}</strong><br/>{booking.items.map((x,i)=><span key={i}>{x.name}{i<booking.items.length-1?" · ":""}</span>)}<br/>Duration: {booking.duration} min · Total: ${Number(booking.total).toFixed(2)}</div>
   {!booking.canChange && <div className="error">The 24-hour change/cancellation window has passed. Please contact the studio; a late-notice fee may apply.</div>}
   <div className="manageActions"><button type="button" className="btn dangerBtn" onClick={cancel} disabled={loading||!booking.canChange}>Cancel Appointment</button></div>
   <fieldset><legend>Change appointment</legend>
     <label>New date<Calendar value={date} minDate={minDate} duration={booking.duration} onChange={v=>{setDate(v);setStart("")}} /></label>
     <div className="timePickerField"><div className="fieldLabel">New start time</div>{date?<TimeSlots date={date} duration={booking.duration} selected={start} onChange={setStart} blocked={blocked} available={available} loading={loadingSlots}/>:<p className="muted small">Choose a date first.</p>}</div>
     <button type="button" className="btn" onClick={reschedule} disabled={!booking.canChange||!date||!start||loading||loadingSlots}>Confirm Change</button>
   </fieldset>
 </div>}
 </div>
}
