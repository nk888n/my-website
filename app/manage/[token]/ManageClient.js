...
export default function ManageClient({token}){
 ...
 {booking.lateNotice?<div className="error" style={{marginTop:12}}><strong>24-hour notice:</strong> Online cancellation and appointment changes are only available until 24 hours before the appointment.</div>:null}
 {booking.canChange?<><div className="manageActions"><button type="button" className="btn dangerBtn" onClick={cancel} disabled={loading}>Cancel Appointment</button></div><fieldset><legend>Change appointment</legend><label>New date<Calendar value={date} minDate={minDate} duration={booking.duration} ignoreBookingId={booking.id} onChange={v=>{setDate(v);setStart("")}}/></label><div className="timePickerField"><div className="fieldLabel">New start time</div>{date?<TimeSlots date={date} duration={booking.duration} selected={start} onChange={setStart} blocked={blocked} available={available} loading={loadingSlots} ignoreId={booking.id} ignoreStart={booking.start} ignoreDuration={booking.duration}/>:<p className="muted small">Choose a date first.</p>}</div><button type="button" className="btn" onClick={reschedule} disabled={!date||!start||loading||loadingSlots}>Confirm Change</button></fieldset></>:<p className="muted" style={{marginTop:16}}>You can cancel or change your appointment through this private link up to 24 hours before the appointment.</p>}
 </div>}
 </div>
}