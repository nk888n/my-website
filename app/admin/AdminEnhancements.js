...
export default function AdminEnhancements(){
 ...
 useEffect(()=>{
  const onClick=e=>{
   ...
   if(card){setTimeout(()=>{const el=document.querySelector(".profileModal");if(!el)return;const email=el.querySelector('input[type="email"]')?.value||"";const name=el.querySelector(".modalHeader h3")?.textContent||"";fetch("/api/admin",{headers:{"x-admin-pin":pin()},cache:"no-store"}).then(r=>r.json()).then(j=>{const list=j.customers||[];setCustomers(list);const c=list.find(x=>x.email?.toLowerCase()===email.toLowerCase()&&x.name===name)||list.find(x=>x.email?.toLowerCase()===email.toLowerCase());setModal({el,customer:c||null})}).catch(()=>setModal({el,customer:null}))},0);return}
 ...
 useEffect(()=>{const el=modal?.el;if(!el)return;el.classList.add("dossier-mode");return()=>el.classList.remove("dossier-mode")},[modal]);
