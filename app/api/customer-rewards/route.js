...
export async function GET(req){
  try{
    ...
    const {data,error}=await c.from("winners").select("id,prize_name,service_id,starts_at,expires_at,max_uses,uses,active").eq("customer_id",customerId).eq("active",true).lte("starts_at",now.toUTC().toISO()).order("created_at",{ascending:false});
