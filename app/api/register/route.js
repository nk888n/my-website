...
async function audit(c,action,entity_type,entity_id,email,details={}){const {error}=await c.from("admin_audit_log").insert({action,entity_type,entity_id:String(entity_id||""),email:email||null,details});if(error)console.error("REGISTER AUDIT FAILED",error)}
...
export async function POST(req){
  try{
    ...
    await audit(c,"customer_profile_created","customer",customer.id,customer.email,{name:customer.name,source:"register"});
